#include "uas_scheduler/CompleteTestMissionScheduler.h"
...
CompleteTestMissionScheduler::CompleteTestMissionScheduler(std::string configPath, std::string csvPath) : Scheduler("uas_trailing_mission") {
    ...
    psSubscription_ = this->create_subscription<sensor_msgs::msg::Image>(
        "/camera/image_raw", qos, std::bind(&CompleteTestMissionScheduler::callbackPS, this, std::placeholders::_1)
    );

    stateSubscription_ = this->create_subscription<px4_msgs::msg::VehicleLocalPosition>(
        "/fmu/out/vehicle_local_position", qos, std::bind(&CompleteTestMissionScheduler::callbackState, this, std::placeholders::_1)
    );

    attitudeSubscription_ = this->create_subscription<px4_msgs::msg::VehicleAttitude>(
        "/fmu/out/vehicle_attitude", qos, std::bind(&CompleteTestMissionScheduler::callbackAttitude, this, std::placeholders::_1)
    );
    ...
    timer_ = create_wall_timer(std::chrono::milliseconds(50), std::bind(&CompleteTestMissionScheduler::timerCallback, this));
}

bool CompleteTestMissionScheduler::isUASStopped(RGV rgv) {
...
bool CompleteTestMissionScheduler::isRGVCoarseLocalized(RGV rgv) {
...
bool CompleteTestMissionScheduler::areRGVsInFrame() {
...
void CompleteTestMissionScheduler::publishRGV1State(){
    std_msgs::msg::Float64MultiArray msg;
    msg.data = {rgv1_.state_.ix_, rgv1_.state_.iy_, rgv1_.state_.iz_};
    rgv1StatePublisher_->publish(msg);
}

void CompleteTestMissionScheduler::publishRGV2State(){
    std_msgs::msg::Float64MultiArray msg;
    msg.data = {rgv2_.state_.ix_, rgv2_.state_.iy_, rgv2_.state_.iz_};
    rgv2StatePublisher_->publish(msg);
}

void CompleteTestMissionScheduler::callbackPS(const sensor_msgs::msg::Image::SharedPtr psMsg) {
    psMsgReceived_ = true;
    imageConvertPS(psMsg);
}

void CompleteTestMissionScheduler::imageConvertPS(const sensor_msgs::msg::Image::SharedPtr sImg)
{
    try {
...
void CompleteTestMissionScheduler::savePSFrame()
{
    if (!psFrame_.empty()) {
...
void CompleteTestMissionScheduler::timerCallback(){
    ...
    std::cout<< "RGV 2 Blobs: " << rgv2CVData_.blobs.size() << ". ";
    std::cout << std::endl;
    ...
    if(rgv1_.currentPhase_ == "exploration" && currentPhase_ == "exploration" && rgv1CVData_.blobs.size() > 0 && uas_.state_.iz_ <= minHeight_){
        ...
        else if(rgv1_.isFineLocalizing_){
        ...
        if (isUASStopped(rgv2_)) {
            ...
            currentPhase_ = "coarse";
            rgv2_.phaseStartTime_ = std::chrono::system_clock::now();
            rgv2_.isCoarseLocalizing_ = true;
            goalState_ = coarsePhase_->generateDesiredState(rgv2CVData_, uas_.state_);
        }
        else{
            goalState_ = trailingPhase_->generateDesiredState(rgv2CVData_, uas_.state_);
...
int main(int argc, char *argv[])
{
    ...
    std::filesystem::path configPath = currentPath  / "configurations" / "CompleteTestMissionSchedulerConfig.yaml";
	...
	rclcpp::spin(std::make_shared<CompleteTestMissionScheduler>(configPath, outputPath));
	rclcpp::shutdown();
	return 0;
}