#include "uas_scheduler/TrailingTestMissionScheduler.h"
...
TrailingTestMissionScheduler::TrailingTestMissionScheduler(std::string configPath, std::string csvPath) : Scheduler("uas_trailing_mission") {
    ...
    psSubscription_ = this->create_subscription<sensor_msgs::msg::Image>(
        "/camera/image_raw", qos, std::bind(&TrailingTestMissionScheduler::callbackPS, this, std::placeholders::_1)
    );

    stateSubscription_ = this->create_subscription<px4_msgs::msg::VehicleLocalPosition>(
        "/fmu/out/vehicle_local_position", qos, std::bind(&TrailingTestMissionScheduler::callbackState, this, std::placeholders::_1)
    );

    attitudeSubscription_ = this->create_subscription<px4_msgs::msg::VehicleAttitude>(
        "/fmu/out/vehicle_attitude", qos, std::bind(&TrailingTestMissionScheduler::callbackAttitude, this, std::placeholders::_1)
    );
    ...
    timer_ = create_wall_timer(std::chrono::milliseconds(50), std::bind(&TrailingTestMissionScheduler::timerCallback, this));
}

bool TrailingTestMissionScheduler::isUASStopped(RGV rgv) {
...
bool TrailingTestMissionScheduler::isRGVCoarseLocalized(RGV rgv) {
...
bool TrailingTestMissionScheduler::areRGVsInFrame() {
...
void TrailingTestMissionScheduler::publishRGV1State(){
    std_msgs::msg::Float64MultiArray msg;
    msg.data = {rgv1_.state_.ix_, rgv1_.state_.iy_, rgv1_.state_.iz_};
    rgv1StatePublisher_->publish(msg);
}

void TrailingTestMissionScheduler::publishRGV2State(){
    std_msgs::msg::Float64MultiArray msg;
    msg.data = {rgv2_.state_.ix_, rgv2_.state_.iy_, rgv2_.state_.iz_};
    rgv2StatePublisher_->publish(msg);
}

void TrailingTestMissionScheduler::callbackPS(const sensor_msgs::msg::Image::SharedPtr psMsg) {
    psMsgReceived_ = true;
    imageConvertPS(psMsg);
}

void TrailingTestMissionScheduler::imageConvertPS(const sensor_msgs::msg::Image::SharedPtr sImg)
{
    try {
...
void TrailingTestMissionScheduler::savePSFrame()
{
    if (!psFrame_.empty()) {
...
void TrailingTestMissionScheduler::timerCallback(){
...
int main(int argc, char *argv[])
{
    ...
    std::filesystem::path configPath = currentPath  / "configurations" / "TrailingTestMissionSchedulerConfig.yaml";
	...
	rclcpp::spin(std::make_shared<TrailingTestMissionScheduler>(configPath, outputPath));
	rclcpp::shutdown();
	return 0;
}