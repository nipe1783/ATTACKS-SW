#include "uas_scheduler/WaypointMissionScheduler.h"
...
WaypointMissionScheduler::WaypointMissionScheduler(std::string configPath) : Scheduler("uas_waypoint_mission") {
    rmw_qos_profile_t qos_profile = rmw_qos_profile_sensor_data;
    auto qos = rclcpp::QoS(rclcpp::QoSInitialization(qos_profile.history, 5), qos_profile);

    stateSubscription_ = this->create_subscription<px4_msgs::msg::VehicleLocalPosition>(
        "/fmu/out/vehicle_local_position", qos, std::bind(&WaypointMissionScheduler::callbackState, this, std::placeholders::_1)
    );

    attitudeSubscription_ = this->create_subscription<px4_msgs::msg::VehicleAttitude>(
        "/fmu/out/vehicle_attitude", qos, std::bind(&WaypointMissionScheduler::callbackAttitude, this, std::placeholders::_1)
    );
    ...
    timer_ = create_wall_timer(std::chrono::milliseconds(50), std::bind(&WaypointMissionScheduler::timerCallback, this));
}

void WaypointMissionScheduler::timerCallback(){
...
int main(int argc, char *argv[])
{
    ...
    std::filesystem::path configPath = currentPath  / "configurations" / "WaypointMissionSchedulerConfig.yaml";
    std::cout<<configPath<<std::endl;
    std::cout << "Starting UAS Waypoint mission node..." << std::endl;
    setvbuf(stdout, NULL, _IONBF, BUFSIZ);
    rclcpp::init(argc, argv);
	rclcpp::spin(std::make_shared<WaypointMissionScheduler>(configPath));
	rclcpp::shutdown();
	return 0;
}