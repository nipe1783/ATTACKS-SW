...
class UasTrailing : public rclcpp::Node
{
public:
    UasTrailing() : Node("uas_sensor_listener"), image_count_(0)
    {
        // Create a subscription to the camera topic
        image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/camera/image_raw", 10, std::bind(&UasTrailing::image_callback, this, std::placeholders::_1));
        
        // Set up a timer to save images once per second
        timer_ = this->create_wall_timer(1000ms, std::bind(&UasTrailing::save_image, this));
    }
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<UasTrailing>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
